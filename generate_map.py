...
from SaliencyModel.BackProp_my import I_gradient, attribution_objective, Path_gradient
from SaliencyModel.BackProp_my import saliency_map_PG as saliency_map
from SaliencyModel.BackProp_my import GaussianBlurPath
from SaliencyModel.utils import grad_norm, IG_baseline, interpolation, isotropic_gaussian_kernel
model_name = 'RNAN@Base'
model = load_model(model_name)  # You can Change the model name to load different model

# Load test image
window_size = 15  # Define windoes_size of D
...
pil.save(os.path.join('result', model_name+'.bmp'))
